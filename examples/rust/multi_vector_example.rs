use engramdb::{
    ...
    embeddings::{EmbeddingError, EmbeddingService, MockMultiVectorProvider},
    error::EngramDbError,
    vector::{
        MultiVectorIndex, MultiVectorIndexConfig, MultiVectorSimilarityMethod, VectorAlgorithm,
        VectorIndexConfig,
    },
    StorageType,
};
...
fn main() -> std::result::Result<(), ExampleError> {
    ...
    println!(
        "Multi-vector dimensions: {:?}",
        service.multi_vector_dimensions()
    );
    ...
    for (i, doc) in documents.iter().enumerate() {
        ...
        // Get dimensions before moving
        let mv_num_vectors = node_mv
            .multi_vector_embeddings()
            .map(|mv| mv.num_vectors())
            .unwrap_or(0);
        let mv_dimensions = node_mv
            .multi_vector_embeddings()
            .map(|mv| mv.dimensions())
            .unwrap_or(0);

        multi_vector_nodes.push(node_mv);

        println!(
            "- Document {}: Created single and multi-vector nodes",
            i + 1
        );
        println!("  └ Single vector dimensions: {}", sv_dimensions);
        println!(
            "  └ Multi vector: {} vectors of {} dimensions each",
    ...
    println!(
        "Added {} multi-vector nodes to index",
        multi_vector_nodes.len()
    );
