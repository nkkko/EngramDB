...
The vector index maintains these embeddings and provides efficient similarity search using cosine similarity. EngramDB now supports two vector index implementations:

- **Linear Index (default)**: Simple brute-force search, suitable for small datasets
