...
impl SingleFileStorageEngine {
    ...
    pub fn new<P: AsRef<Path>>(file_path: P) -> Result<Self> {
        ...
        if file_exists {
            ...
            let file = OpenOptions::new()
                ...
                .open(&file_path)
                .map_err(|e| {
                    EngramDbError::Storage(format!("Failed to create database file: {}", e))
                })?;
    ...
    fn initialize_file(mut file: File) -> Result<()> {
        ...
        let serialized = bincode::serialize(&header).map_err(|e| {
            EngramDbError::Serialization(format!("Failed to serialize header: {}", e))
        })?;
        ...
        let index_bytes = bincode::serialize(&empty_index).map_err(|e| {
            EngramDbError::Serialization(format!("Failed to serialize empty index: {}", e))
        })?;
    ...
    fn load_index(file_path: &Path) -> Result<HashMap<Uuid, IndexEntry>> {
        ...
        if file_size < HEADER_SIZE + 8 {
            // Header + at least 8 bytes for index size
            return Err(EngramDbError::Storage(
                "Database file is too small or corrupt".to_string(),
            ));
        }
        ...
        let header: DatabaseHeader = bincode::deserialize(&header_bytes).map_err(|e| {
            EngramDbError::Serialization(format!("Failed to deserialize header: {}", e))
        })?;

        // Validate magic bytes
        if header.magic != MAGIC {
            return Err(EngramDbError::Storage(
                "Invalid database file format".to_string(),
            ));
        }
        ...
        if header.index_offset + 8 + index_size > file_size {
            return Err(EngramDbError::Storage(
                "Database file is corrupt - index size exceeds file size".to_string(),
            ));
        }
        ...
        // Deserialize index
        let index: HashMap<Uuid, IndexEntry> = bincode::deserialize(&index_bytes).map_err(|e| {
            EngramDbError::Serialization(format!("Failed to deserialize index: {}", e))
        })?;
    ...
    fn update_index(&self) -> Result<()> {
        ...
        let mut header: DatabaseHeader = bincode::deserialize(&header_bytes).map_err(|e| {
            EngramDbError::Serialization(format!("Failed to deserialize header: {}", e))
        })?;

        // Serialize the index
        let index_bytes = bincode::serialize(&self.index).map_err(|e| {
            EngramDbError::Serialization(format!("Failed to serialize index: {}", e))
        })?;
        ...
        let updated_header_bytes = bincode::serialize(&header).map_err(|e| {
            EngramDbError::Serialization(format!("Failed to serialize header: {}", e))
        })?;
    ...
    pub fn compact(&mut self) -> Result<()> {
        ...
        let temp_file = OpenOptions::new()
            ...
            .open(&temp_path)
            .map_err(|e| {
                EngramDbError::Storage(format!("Failed to create temporary file: {}", e))
            })?;
        ...
        std::fs::rename(&temp_path, &self.file_path).map_err(|e| {
            EngramDbError::Storage(format!("Failed to replace database file: {}", e))
        })?;
...
impl StorageEngine for SingleFileStorageEngine {
    fn save(&mut self, node: &MemoryNode) -> Result<()> {
        ...
        let node_bytes = bincode::serialize(node).map_err(|e| {
            EngramDbError::Serialization(format!("Failed to serialize node: {}", e))
        })?;
        ...
        if file.metadata().map(|m| m.len()).unwrap_or(0) == 0 {
            ...
            file = OpenOptions::new()
                ...
                .open(&self.file_path)
                .map_err(|e| {
                    EngramDbError::Storage(format!("Failed to reopen database file: {}", e))
                })?;
        }
    ...
    fn load(&self, id: Uuid) -> Result<MemoryNode> {
        ...
        // Deserialize node
        let node: MemoryNode = bincode::deserialize(&node_bytes).map_err(|e| {
            EngramDbError::Serialization(format!("Failed to deserialize node: {}", e))
        })?;
