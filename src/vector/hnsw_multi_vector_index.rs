...
use crate::Result;
use crate::vector::{
    cosine_similarity, HnswConfig, MultiVectorIndexConfig,
    MultiVectorSimilarityMethod, VectorSearchIndex,
};
...
impl HnswMultiVectorIndex {
    ...
    pub fn add_multi_vector(&mut self, id: Uuid, mut multi_vec: MultiVectorEmbedding) -> Result<()> {
        ...
        let similarity = self.compute_similarity(&representative, entry_point_id)?;
        ...
        for level_idx in (0..=level.min(self.max_level)).rev() {
            ...
            let nearest = self.search_level(&representative, curr_node_id, curr_dist, level_idx, 1)?;
                    ...
                    if let Some(node) = self.nodes.get_mut(&neighbor_id) {
                        if node.connections.len() > level_idx {
                            ...
                            if node.connections[level_idx].len() > self.config.hnsw_config.m {
                                ...
                                let selected = self.select_neighbors(&candidates, self.config.hnsw_config.m);
    ...
    pub fn search_multi_vector(
        ...
        let candidates = self.search_level(&representative, curr_node_id, curr_dist, 0, ef)?;
        ...
        for candidate in candidates {
            if let Some(multi_vec) = self.embeddings.get(&candidate.id) {
                let similarity = match self.config.multi_vector_config.similarity_method {
                    ...
                    MultiVectorSimilarityMethod::LateInteraction => query.late_interaction_score(multi_vec),
                };
...
mod tests {
    ...
    fn test_search_multi_vector() {
        ...
        for pooling in &[PoolingStrategy::MeanPool, PoolingStrategy::MaxPool, PoolingStrategy::Centroid] {
            let mut config = HnswMultiVectorConfig::default();
            ...
            // Search
            let results = index_with_strategy.search_multi_vector(&query, 5, 0.0).unwrap();
