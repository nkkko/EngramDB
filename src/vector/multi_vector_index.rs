...
impl MultiVectorIndex {
    ...
    pub fn add(&mut self, node: &MemoryNode) -> Result<()> {
        ...
        if let Some(multi_vec) = node.multi_vector_embeddings() {
            ...
            if self.config.quantize {
                multi_vec.quantize().map_err(|e| EngramDbError::Vector(e.to_string()))?;
            }
    ...
    pub fn search(&self, query: &MultiVectorEmbedding, limit: usize, threshold: f32) -> Result<Vec<(Uuid, f32)>> {
...
impl VectorSearchIndex for MultiVectorIndex {
    ...
    fn get(&self, id: Uuid) -> Option<&Vec<f32>> {
        ...
        self.multi_vectors.get(&id).and_then(|multi_vec| {
            multi_vec.vectors().first().map(|v| v as &Vec<f32>)
        })
    }
}
