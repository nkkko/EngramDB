...
pub use multi_vector::{
    MockMultiVectorProvider, MultiVectorEmbedding, MultiVectorError, MultiVectorProvider,
};
...
pub mod embedding_service {
    ...
    impl fmt::Display for EmbeddingError {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            match self {
                ...
                EmbeddingError::GenerationFailed(msg) => {
                    write!(f, "Failed to generate embeddings: {}", msg)
                }
    ...
    impl EmbeddingService {
        ...
        pub fn new_mock_multi_vector(dimensions: usize, num_vectors: usize) -> Self {
            ...
            let multi_provider =
                multi_vector::MockMultiVectorProvider::new(dimensions, num_vectors);
        ...
        pub fn new_with_model(model_name: Option<&str>) -> Self {
            ...
            match providers::ModelProvider::new(model_name) {
                ...
                Err(err) => {
                    log::warn!(
                        "Failed to initialize model provider: {}. Falling back to mock provider.",
                        err
                    );
        ...
        pub fn with_multi_vector_model(model_type: models::EmbeddingModel) -> Self {
            if !model_type.is_multi_vector() {
                log::warn!(
                    "Model {} is not a multi-vector model. Using standard model provider.",
                    model_type.model_id()
                );
            ...
            match providers::ModelProvider::new(model_type.model_id()) {
                ...
                Err(err) => {
                    ...
                    Self::new_mock_multi_vector(
                        model_type.dimensions(),
                        model_type.typical_vector_count(),
                    )
                }
            }
        }
        ...
        pub fn generate_for_document(
            ...
            let provider = self.get_provider()?;
            provider
                .generate_for_document(text, category)
                .map_err(|e| EmbeddingError::GenerationFailed(e.to_string()))
        }
        ...
        pub fn generate_multi_vector(&self, text: &str) -> EmbeddingResult<MultiVectorEmbedding> {
            ...
            provider
                .generate_multi_vector(text)
                .map_err(|e| EmbeddingError::GenerationFailed(e.to_string()))
        }

        /// Generate multi-vector embeddings for a document
        #[cfg(feature = "embeddings")]
        pub fn generate_multi_vector_for_document(
            &self,
            text: &str,
            category: Option<&str>,
            ...
            let provider = self.get_multi_vector_provider()?;
            provider
                .generate_multi_vector_for_document(text, category)
                .map_err(|e| EmbeddingError::GenerationFailed(e.to_string()))
        }

        /// Generate multi-vector embeddings for a query
        #[cfg(feature = "embeddings")]
        pub fn generate_multi_vector_for_query(
            &self,
            text: &str,
        ) -> EmbeddingResult<MultiVectorEmbedding> {
            let provider = self.get_multi_vector_provider()?;
            provider
                .generate_multi_vector_for_query(text)
                .map_err(|e| EmbeddingError::GenerationFailed(e.to_string()))
        }
        ...
        #[cfg(feature = "embeddings")]
        pub fn generate_random_multi_vector(
            &self,
        ...
        fn get_provider(&self) -> EmbeddingResult<MutexGuard<Box<dyn Provider + Send>>> {
            self.provider.lock().map_err(|e| {
                EmbeddingError::ProviderUnavailable(format!("Failed to acquire lock: {}", e))
            })
        }

        // Get a lock on the multi-vector provider
        #[cfg(feature = "embeddings")]
        fn get_multi_vector_provider(
            &self,
        ) -> EmbeddingResult<MutexGuard<Box<dyn MultiVectorProvider + Send>>> {
            if let Some(ref provider) = self.multi_vector_provider {
                provider.lock().map_err(|e| {
                    EmbeddingError::ProviderUnavailable(format!("Failed to acquire lock: {}", e))
                })
            } else {
                Err(EmbeddingError::ProviderUnavailable(
                    "Multi-vector provider not available".to_string(),
                ))
            }
        }
    }
