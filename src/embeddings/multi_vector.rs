...
impl MultiVectorEmbedding {
    ...
    pub fn new_normalized(vectors: Vec<Vec<f32>>) -> Result<Self, MultiVectorError> {
        ...
        let normalized_vectors = vectors.iter().map(|v| normalize_vector(v)).collect();
...
mod tests {
    ...
    fn test_multi_vector_similarity() {
        let vectors1 = vec![vec![1.0, 0.0, 0.0], vec![0.0, 1.0, 0.0]];

        let vectors2 = vec![vec![0.8, 0.2, 0.0], vec![0.0, 0.0, 1.0]];
    ...
    fn test_mock_provider() {
        ...
        let result = provider
            .generate_multi_vector_for_query("Query text")
            .unwrap();
        assert_eq!(result.dimensions(), 96);
