    ...
    pub fn new(model_name: &str) -> Result<Self, Box<dyn Error>> {
        ...
        #[cfg(feature = "python")]
        {
            ...
            Python::with_gil(|py| {
                ...
                let tokenizer =
                    auto_tokenizer.call_method("from_pretrained", (model_id,), Some(kwargs))?;
    ...
    fn generate_for_document(
        ...
        #[cfg(feature = "python")]
        {
            ...
            Python::with_gil(|py| {
                ...
                if cuda_available {
                    let device = torch.getattr("cuda")?.call_method0("current_device")?;
                ...
                let outputs = model.call_method(
                    "__call__",
                    (),
                    Some(inputs.as_ref(py).downcast::<PyDict>()?),
                )?;
                ...
                let normalized = functional.call_method(
                    "normalize",
                    (embeddings,),
                    Some(PyDict::new(py).apply(|d| {
                ...
                let numpy_embeddings = normalized
                    .call_method0("cpu")?
                    .call_method0("detach")?
                    .call_method0("numpy")?
...
impl MultiVectorProvider for ModelProvider {
    ...
    fn generate_multi_vector_for_document(
        &self,
        _text: &str,
        _category: Option<&str>,
    ) -> Result<MultiVectorEmbedding, Box<dyn Error>> {
        ...
        #[cfg(feature = "python")]
        {
            ...
            let formatted_text = match self.model_type {
                ...
                _ => {
                    ...
                    match _category {
                        Some(cat) => {
                            format!("Represent the {} document for retrieval: {}", cat, _text)
                        }
                        None => format!("Represent the document for retrieval: {}", _text),
                    }
                }
            ...
            Python::with_gil(|py| {
                ...
                // Convert to numpy
                let np_normalized = batch_normalized
                    .call_method0("detach")?
                    .call_method0("numpy")?;
                ...
                let valid_indices =
                    numpy.call_method1("where", (np_mask.call_method1("__eq__", (1,))?,))?;
                let valid_vectors = np_normalized.call_method(
                    "__getitem__",
                    (valid_indices.call_method("__getitem__", (0,))?,),
                    None,
                )?;
                ...
                let num_tokens = valid_vectors.call_method0("__len__")?.extract::<usize>()?;

                // Limit tokens for efficiency if needed
                let max_tokens =
                    if self.model_type == crate::embeddings::EmbeddingModel::JinaColBERTv2 {
                ...
                MultiVectorEmbedding::new(token_vectors).map_err(|e| e.to_string().into())
            })
        }
    ...
    fn generate_multi_vector_for_query(
        &self,
        _text: &str,
    ) -> Result<MultiVectorEmbedding, Box<dyn Error>> {
        if !self.model_type.is_multi_vector() {
