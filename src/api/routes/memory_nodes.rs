...
pub fn list_nodes(
    ...
    let node_ids = db.list_all().map_err(|e| ApiError::from_engramdb_error(e))?;
...
pub fn create_node(
    ...
    let node_id = db.save(&node).map_err(|e| ApiError::from_engramdb_error(e))?;
...
pub fn get_node(
    ...
    let node_id = Uuid::parse_str(&node_id)
        .map_err(|_| ApiError::bad_request("Invalid node ID format"))?;
...
pub fn update_node(
    ...
    let mut node = db.load(node_id).map_err(|e| ApiError::from_engramdb_error(e))?;
    ...
    db.save(&node).map_err(|e| ApiError::from_engramdb_error(e))?;
